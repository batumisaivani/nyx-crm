...
/**
 * NewBookingInterface - Modal wrapper for creating new bookings
 * Shows specialist selection first, then the scheduling card
 */
export default function NewBookingInterface({ isOpen, onClose, facilityId, prefilledSpecialistId = null, prefilledDateTime = null }) {
  const [selectedSpecialistId, setSelectedSpecialistId] = useState(prefilledSpecialistId)
  ...
  useEffect(() => {
    if (prefilledSpecialistId) {
      setSelectedSpecialistId(prefilledSpecialistId)
    } else {
      setSelectedSpecialistId(null)
    }
  }, [prefilledSpecialistId, isOpen])
        ...
        <div className="sticky top-0 bg-[#120025] border-b border-purple-700 px-4 py-3 flex items-center justify-between z-10">
          <div className="flex items-center gap-3">
            ...
            <h2 className="text-lg font-bold text-white font-[Calibri,sans-serif]">
              {selectedSpecialistId ? 'Schedule Appointment' : 'Select Specialist'}
            </h2>
          </div>
          <button
            onClick={handleClose}
                ...
                {loading ? (
                  ...
                  <>
                    <p className="text-purple-200 mb-4 text-center text-sm">
                      Choose a specialist to book an appointment
                    </p>
