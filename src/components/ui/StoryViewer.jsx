...
function StoryThumbnail({ stories, username, viewedIndices, onClick, className }) {
  ...
  return (
    <button
      ...
      className={cn(
        ...
        "focus-visible:ring-2 focus-visible:ring-purple-500 focus-visible:ring-offset-2 rounded-lg",
        className
      )}
    ...
    >
        ...
        <div className="absolute inset-[5px] rounded-full bg-black p-[2px]">
          <div className="w-full h-full rounded-full overflow-hidden bg-gray-800">
            {lastStory.type === "video" ? (
              ...
              <img
                src={thumbnailImage || lastStory.src}
      ...
      <span className="text-[11px] text-gray-400 truncate max-w-[76px]">
        {username}
      </span>
    </button>
  );
...
function StoryContent({ story, isMuted, isInitialLoading, isBuffering, onVideoReady, onVideoTimeUpdate, onVideoWaiting, onVideoPlaying, onVideoEnded, onImageLoad, videoRef }) {
...
function StoryViewerModal({ stories, username, avatar, timestamp, initialIndex, viewedIndices, onClose, onStoryChange }) {
      ...
      >
          ...
          <div className="flex items-center justify-between px-4 mt-3">
            ...
            <div className="flex items-center gap-2">
              ...
              {currentStory.type === "video" && (
                <button
                  type="button"
                  className="inline-flex items-center justify-center rounded-md h-8 w-8 text-white hover:bg-white/20 transition-colors"
                  onClick={(e) => {
                    e.stopPropagation();
        ...
        <div className="hidden md:flex absolute inset-y-0 left-0 right-0 items-center justify-between pointer-events-none px-4">
          <button
            type="button"
            className={cn(
              "inline-flex items-center justify-center h-10 w-10 rounded-full bg-white/10 text-white hover:bg-white/20 transition-colors pointer-events-auto",
          ...
          <button
            type="button"
            className="inline-flex items-center justify-center h-10 w-10 rounded-full bg-white/10 text-white hover:bg-white/20 transition-colors pointer-events-auto"
            onClick={(e) => {
              e.stopPropagation();
              goToNext();
            }}
...
export function StoryViewer({ stories, username, avatar, timestamp, onStoryView, onAllStoriesViewed, className }) {
